// utils/matcher.js - Fixed version
...
async function processReport(userId, reportType, records) {
  ...
  for (const record of records) {
    try {
      ...
      // Find existing using findFirst (works reliably with compound fields)
      const existing = await prisma.order.findFirst({
        where: { orderItemId: orderItemId, userId: userId },
      });
...
async function recalculateProfits(userId) {
  ...
  const skuMap = {};
  for (const s of skuList) skuMap[s.skuId.toLowerCase()] = s.purchasePrice;
  const matched = await prisma.order.findMany({ where: { userId, isMatched: true, skuId: { not: null } } });
